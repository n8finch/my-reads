...
const MyReadsFilterApp = ({ attributes }) => {
  ...
  const getFilteredPosts = () => {
    return postsArray.filter(
      (post) =>
        ...
        post._myreads_format.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const handleFilterChange = (selectedGenre) => {
    ...
    if (selectedGenre === 'All') {
      ...
      setPostsArray(
        getBaseFilteredPosts().filter((post) => post._myreads_isFavorite)
      );
    } else {
      setPostsArray(
  ...
  return (
    <>
      ...
      {search && (
        <p>
          {__(
            `Showing ${getFilteredPosts().length} results for "${search}"`,
            'my-reads'
          )}
        </p>
      )}
